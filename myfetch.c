...
static void init_colors(bool grey, const char *color_hex) {
    if (grey) {
        ...
        /* All palette entries point to the same custom colour */
...
static size_t vis_len(const char *s) {
    ...
    for (const unsigned char *p = (const unsigned char *)s; *p; p++) {
        ...
        if ((*p & 0xC0) != 0x80)  len++;  /* skip UTF-8 continuation bytes */
    }
    return len;
}
...
static void gather(void) {
    ...
    {
        run_cmd("uptime -p 2>/dev/null", g_uptime, sizeof(g_uptime));
        if (strncmp(g_uptime, "up ", 3) == 0)
            memmove(g_uptime, g_uptime+3, strlen(g_uptime+3)+1);
        else if (!g_uptime[0])
            strcpy(g_uptime, "unknown");
    }
        ...
        if (!got_res) {
            ...
            if (glob("/sys/class/drm/card*-*/mode", 0, NULL, &gl) == 0) {
                ...
                for (size_t gi = 0; gi < gl.gl_pathc && nd < 2; gi++) {
                    /* verify connector is connected via status file */
                    char spath[512];
    ...
    {
        char model[200] = "", mhz[32] = "";
        run_cmd("lscpu | grep 'Model name:' | cut -d: -f2 | xargs",
                model, sizeof(model));
        run_cmd("lscpu | awk -F: '/CPU MHz/ {printf \"%.1f\", $2/1000; exit}'",
                mhz, sizeof(mhz));
        if (mhz[0]) snprintf(g_cpu, sizeof(g_cpu), "%s @ %s GHz", model, mhz);
        else        snprintf(g_cpu, sizeof(g_cpu), "%s @ unknown",  model);
    }
...

    {
        ...
        run_cmd("free -h | awk '/^Mem:/ {print $3}'", used,  sizeof(used));
        run_cmd("free -h | awk '/^Mem:/ {print $2}'", total, sizeof(total));
        snprintf(g_ram, sizeof(g_ram), "%s / %s", used, total);
    }
...
static void build_ascii_ubuntu(void) {
    const char *or=SORANGE, *r=RESET_C;
    ALN();
    AL("              %s===========%s",                              or,r);
    AL("          %s===================%s",                          or,r);
    AL("        %s=======================%s",                        or,r);
...
static void build_ascii_void(void) {
    /* No colour codes – uses literal text only */
    AL("                -------");
    AL("              -----------");
    AL("               -----------");
    AL("            *   --   ------");
...
/* Used for distros whose ID_LIKE contains ubuntu or debian */
static void build_ascii_deb_ubuntu(void) {
    const char *or=SORANGE, *r=RESET_C;
    ALN();
    AL("              %s===========%s",                              or,r);
    AL("          %s===================%s",                          or,r);
        ...
        if (i < box_h) {
            if (i == 0) {
                snprintf(right, sizeof(right),
                         "%s\xe2\x95\xad%s%s\xe2\x95\xae%s",
                         INFO_COLOR, RESET_C, INFO_COLOR, RESET_C);
                /* append border between the corners */
